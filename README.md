...
生产者需要启用幂等性、重试机制，并设置 ack=all。重试机制有两种情况：一种是处理网络闪断后的重试，另一种是处理消息发送到 broker 失败的重试。对于后者，当重试次数达到上限时，可以采取补救措施，比如将消息写入死信队列以待后续处理，或将其存入数据库以便后续对账，甚至触发报警等。

消费者端则需要实现幂等校验，并关闭自动偏移量提交。幂等表会记录每个消息的 ID，只有当幂等表中没有该消息时，才会进行处理。消息处理完成后，才会提交偏移量，并将新的消息 ID 写入幂等表。
